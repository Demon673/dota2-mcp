...
这样多个 MCP 客户端（多个 AI agent / 多个会话）能同时连同一个 relay，共享对 Dota 2 `:29000` 的独占连接。守护进程状态放在 `os.tmpdir()/dota2-mcp/`（失败时 fallback `~/.dota2-mcp/`）：`relay.lock`（原子抢锁）、`relay.pid`、`relay.token`（0600，瘦客户端 `HELLO` 时校验）、`relay.log`。无客户端连接空闲 5 分钟后守护进程自动退出。

relay 只在 **vconsole2 GUI 连接到 :29001 之后**才会去连 Dota 2 的 `:29000`；GUI 断开时释放 `:29000`。
...
**注意**：relay 只在 **vconsole2 GUI 连接到 :29001 之后**才会去连 Dota 2 的 `:29000`；GUI 断开时会释放 `:29000`。
