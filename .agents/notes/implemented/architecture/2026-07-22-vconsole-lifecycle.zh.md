# Agent Note: vconsole relay 连接生命周期（GUI 门控连接 + 活性探测 + 初始化帧重放 + 空闲守卫）
...
4. **空闲退出切断生命线**：无瘦客户端且无 GUI 满 5 分钟守护进程退出，`29001/29002` 消失，vconsole 连 TCP 都没地方连 —— 哪怕 Dota 正在运行（用户正在开发）。

## Decision

relay 仅在 vconsole2 GUI 附着 `:29001` 期间连接 Dota 2 `:29000`。无 GUI 时不持有长连接——每秒探测一次引擎就绪（连上即断，不保留 socket）。GUI 附着才触发真正的连接，此后 GUI 在时断线每 2s 重连；GUI 关闭立刻断开 `:29000`，引擎的 AssetBrowser vconsole 按钮随之恢复可用。四项机制兜住生命周期（全部在 `src/tools/vcon-relay.ts`，守护进程与内嵌模式同生效）：
...
- **29000 租约 / 引用计数** — 输了：单一 GUI 附着是唯一的连接触发点，没有可计数的对象，死亡检测也已由活性探针承担。「没窗口 = 没连接 = 没工具」保持物理为真。
