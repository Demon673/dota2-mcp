...
`prose-standard`, `trim-cot-leakage`, `translate-docs`, `archive-agent-notes`, and `code-review` are not built in. Their global versions are authoritative, and in-repo references to them are bare text ("the global prose-standard skill"). `doc-standards` is not built in either: maintenance skills live in the maintainer's global skill directory. `skills/` ships only runtime knowledge. `dota2-game-phases` and `dota2-runtime-dev` stay built in because `scripts/test-mcp-offline.mjs` and the stuck-phase guidance in `src/index.ts` pin them; the global `dota2-custom-game-dev` covers addon development but neither the MCP phase-advance SOP nor the runtime hot-reload model.
